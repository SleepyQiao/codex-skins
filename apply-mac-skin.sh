...
set -euo pipefail
payload_file=''''
cleanup_payload(){ [[ -n ${payload_file:-} ]] && rm -f -- "$payload_file"; }
die(){ printf '%s
' "$*" >&2; exit 1; }
