...
payload_field(){ /usr/bin/osascript -l JavaScript - "$1" <<''JXA''
ObjC.import("Foundation");function run(a){var d=$.NSFileHandle.fileHandleWithStandardInput.readDataToEndOfFile,p=JSON.parse(ObjC.unwrap($.NSString.alloc.initWithDataEncoding(d,$.NSUTF8StringEncoding))),v=p[a[0]];if(typeof v!=="string")throw Error("Invalid renderer payload field");$.NSFileHandle.fileHandleWithStandardOutput.writeData($.NSString.stringWithString(v).dataUsingEncoding($.NSUTF8StringEncoding))}
JXA
}
